...
pub async fn chat_by_id(
    ...
    let chat_message_pairs = state.chat_repo.retrieve_chat(chat_id)
        .await
        .map_err(|_| ChatError::Other)?;

    let user_chats = state.chat_repo.get_all_chats(current_user.as_ref().unwrap().id)
        .await
        .unwrap();
    ...
    let parsed_pairs = chat_message_pairs.iter()
        .map(|pair| {
...
pub async fn chat_add_message(
    ...
    let update = state
        .tera
        .render("html_updates/add_message.html", &context)
        .unwrap();
    Ok(Html(update))
}
...
pub async fn chat_generate(
    ...
    let key = current_user.unwrap().openai_api_key
