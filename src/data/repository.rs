...
use crate::data::model::{Chat, ChatMessagePair};
...
impl ChatRepository {
    ...
    pub async fn delete_chat(&self,chat_id:i64) -> sqlx::Result<u64> {
        let rows_affected = query!("DELETE FROM chats WHERE id = ?",chat_id)
            .execute(&self.pool)
            .await
            .unwrap()
            .rows_affected();
    ...
    pub async fn create_chat(&self,user_id:i64,name:&str,model:&str) -> sqlx::Result<i64> {
        let chat = sqlx::query!(
            r#"INSERT INTO chats (user_id, name, model)
            VALUES (?, ?, ?) RETURNING id;"#,
            user_id,
            name,
            model
    ...
    pub async fn add_ai_message_to_pair(&self,pair_id:i64,message:&str) -> sqlx::Result<i64> {
        ...
        let message = sqlx::query!(
            r#"INSERT INTO messages (message)
            VALUES (?) RETURNING id;"#,
            message
        ).fetch_one(&mut *tx).await?;
        sqlx::query!(
            r#"UPDATE message_pairs
            SET ai_message_id = ?
            WHERE id = ?;"#,
            message.id,
            pair_id
    ...
    pub async fn add_message_block(&self,chat_id:i64,human_message:&str) -> sqlx::Result<i64> {
        ...
        let message_block = sqlx::query!(
            r#"INSERT INTO message_blocks (chat_id)
            VALUES (?) RETURNING id;"#,
            chat_id
        ).fetch_one(&mut *tx).await?;
        let message = sqlx::query!(
            r#"NSERT INTO messages (message)
            VALUES (?) RETURNING id;"#,
            human_message
        ).fetch_one(&mut *tx).await?;

        let message_pair = sqlx::query!(
            r#"INSERT INTO message_pairs (human_message_id, message_block_id)
            VALUES (?, ?) RETURNING id;"#,
            message.id,
            message_block.id
        ).fetch_one(&mut *tx).await?;
...
mod tests{
    ...
    async fn test_json() {
        ...
        let message_block = repo.add_message_block(chat_id,"Test").await;
        assert!(message_block.is_ok(), "Failed to add message_block");
        let chat_message_pair = repo.retrieve_chat(chat_id).await;
        println!("{:#?}",chat_message_pair)
    }
}